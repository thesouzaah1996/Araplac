/* ============ Config & endpoint ============ */
const API_BASE = "http://localhost:8080"; // ajuste
const ENDPOINT = {
  FORGOT: () => new URL("/api/auth/forgot-password", API_BASE).toString(), // POST { email }
};
...
/* Início: tudo oculto/normal (sem loading em F5) */
toast.hidden = true;
...
form.addEventListener("submit", async (e) => {
  ...
  try{
    ...
    // Só agora mostra o loading (topo)
    showToast("Enviando e-mail…", "loading");

    // Responder 200/204 mesmo se e-mail não existir (não revelar existência)
